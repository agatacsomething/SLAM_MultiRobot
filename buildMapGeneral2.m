function [log_prob2] = buildMapGeneral2(num2do,order,log_prob2, robot,yshiftnew)
    ...
    %         bwwalls(walls)= 1; 
            ...
            for j=1:length(thetas)
                ...
                for k=1:length(shifts)
                    ...
                    [log_prob_temp_y]= getNewTempMap2(ptd3,robot,log_prob2,thetas(j), ...
                ...
                for p=1:length(shifts)
                    ...
                    [log_prob_temp_x]= getNewTempMap2(ptd3,robot,log_prob2,thetas(j), ...
                        robot_num, rob, rob_new,yshiftnew,shifts(shiftyidx),shifts(k));
                ...
                %shiftyidx
                %j
                [log_prob_temp_shift_xy]= getNewTempMap2(ptd3,robot,log_prob2,thetas(j), ...
                    robot_num, rob, rob_new,yshiftnew,shifts(shiftyidx),shifts(shiftxidx));
                
                bwwalls_temp_shift_xy = getWalls2(log_prob_temp_shift_xy);
                
                corr_angles(j) = corr2(bwwalls,bwwalls_temp_shift_xy);
            end
            toc
