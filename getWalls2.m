function [bwwalls_new] = getWalls2(log_prob2)

walls= find(log_prob2>0.1);
bwwalls= zeros(size(log_prob2)); 
bwwalls(walls)= 1; 

%bwwalls= imerode(imdilate(bwwalls,ones(3)),ones(3)); 
...
bwwalls_new= imerode(imdilate(bwwalls_new,ones(3)),ones(3));
