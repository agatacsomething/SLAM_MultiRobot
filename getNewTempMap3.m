...
function [log_prob_temp]= getNewTempMap3(ptd,robot,log_prob2,theta, ...
    robot_num, rob4, rob4_new, yshiftnew, xshiftnew, xshift_test, yshift_test)%, xshift_test, yshift_test)
...
for i=ptd
     A= [cosd(theta), sind(theta), 0, xshift_test; 
         -sind(theta), cosd(theta), 0, yshift_test;
         0, 0, 1, 0;
         0, 0, 0, 1];
