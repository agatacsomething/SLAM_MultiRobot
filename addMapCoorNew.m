...
% 
%     curr_diff_x = abs(xpos_g(1)-xencod(i));
%     curr_diff_y = abs(ypos_g(1)-yencod(i));
%     theta_diff = theta_g(1) - thetas_d(i); 
%     
%     if curr_diff_x<1 && curr_diff_y <1
%         m=1;
%         stdv_new(1:2)= stdv;
%     elseif curr_diff_x>=1 && curr_diff_y>=1 && theta_diff==0
%         m=9;
%         stdv_new(1:5)= stdv;    
%         stdv_new(5:9)= 2*stdv;  
%     elseif curr_diff_x>=1 && curr_diff_y>=1 && theta_diff>0 && thetas_d(i-1)<pi/2
%         m=5;
%         stdv_new(1:m+1)= stdv;
%     elseif curr_diff_x>=1 && curr_diff_y>=1 && theta_diff>0 && thetas_d(i-1)>pi/2 
%         m=20;
%         stdv_new(1:5)= stdv;
%         stdv_new(6:10)= 2*stdv;
%         stdv_new(11:15)= 3*stdv;
%         stdv_new(15:20)= 4*stdv;
% %         stdv_new(21:25)= -stdv;
% %         stdv_new(26:30)= -2*stdv;
% %         stdv_new(31:35)= -3*stdv;
% %         stdv_new(36:40)= -4*stdv;
%     end
        ...
        for j = anglespan
            ...
            if lidar_reading_new(j,2*i) > .1 && lidar_reading_new(j,2*i) < 5
                ...
                if indGood ==1
                    ...
                    if log_odds(inds)<3 %&& log_odds(inds)>-3
                    ...
                    for ii = 1: length(ax)
                        if log_odds(ax(ii),by(ii))>-3 && log_odds(ax(ii),by(ii))<1
    ...
    %compute correlation
        ...
        posY2 = [xis yis zeros(length(yis),1)]';
         ...
         c(:,:,p) = map_correlation(int8(log_prob),x_im,y_im,posY2,x_range,y_range);
   ...
   % [a,b] = mean(mean(c(:)));
    ...
    if abs(xpos_g(o)-x_robot_new_d(i))<2
        x_robot_new_d(i+1) = x_robot_new_d(i);
    else
        x_robot_new_d(i+1) = xpos_g(o);
    end
    
    if abs(ypos_g(o)-y_robot_new_d(i))<2
