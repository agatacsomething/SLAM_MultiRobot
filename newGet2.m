...
xis2 = 106; 
yis2 = 137; 
%function [non_occ_points] = newGet2(x_rob, y_rob, xis2, yis2)
...
slope_cur= (y_diff_cur)/(x_diff_cur);
...
while ((cur_endy ~=yis2)+(cur_endx ~=xis2))~=0
    ...
    if x_diff_cur>0 && y_diff_cur>0 && slope_cur>=1
        non_occ_points(i,1) = non_occ_points(i-1,1)-1; 
        non_occ_points(i,2) = non_occ_points(i-1,2);
        x_ch =1; 
    elseif x_diff_cur>0 && y_diff_cur>0 && slope_cur<1
        non_occ_points(i,1) = non_occ_points(i-1,1)-1; 
        non_occ_points(i,2) = non_occ_points(i-1,2);
        x_ch =1; 
    elseif x_diff_cur<0 && y_diff_cur>0 && abs(slope_cur)>1
        non_occ_points(i,1) = non_occ_points(i-1,1)-1;
        non_occ_points(i,2) = non_occ_points(i-1,2);
        x_ch =1; 
    elseif x_diff_cur<0 && y_diff_cur>0 && abs(slope_cur)<1
